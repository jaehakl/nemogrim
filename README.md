...
│   └── keyframe/               # 장면 단위 영상 라이브러리
│       ├── api/                # FastAPI + SQLite 백엔드
│       ├── ui/                 # React 19 + TypeScript + Vite 프론트엔드
│       └── data/               # 로컬 DB와 썸네일 (Git 제외)
...
### 3. Keyframe - 장면 단위 영상 라이브러리

원본 영상을 복사하지 않고 로컬 경로로 등록하고, 영상별 Scene timestamp와 AI 분석 결과를 관리하는 도구입니다.

#### 주요 기능
- **로컬 영상 색인**: 복수 파일 또는 폴더 전체를 경로 기준으로 중복 없이 등록
- **백그라운드 분석**: FFmpeg로 영상 정보와 대표 썸네일 생성
- **영상 라이브러리**: 카드형 목록, 처리 상태 표시, 무한 스크롤
- **영상 상세 재생**: 브라우저 호환 원본 Range 스트리밍과 비호환 codec 차단
- **Scene 생성**: 현재 프레임 snapshot, CLIP embedding, WD14 prompt·keyword 생성

#### 기술 스택
- **Backend**: FastAPI, SQLAlchemy, SQLite, FFmpeg, PyTorch, OpenAI CLIP, WD14
...
```bash
# 백엔드 설정
cd apps/keyframe/api
poetry install

# 프론트엔드는 저장소 루트 workspace 설치에 포함
cd ../../..
corepack pnpm install
```

Keyframe은 PATH에서 실행 가능한 `ffmpeg`와 `ffprobe`가 필요합니다.
...
#### Keyframe (SQLite)
`apps/keyframe/data/keyframe.sqlite3`에 자동으로 생성되며 별도 설정 불필요
